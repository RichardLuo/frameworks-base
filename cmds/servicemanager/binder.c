...
struct binder_state *binder_open(size_t mapsize)
{
    ...
    fprintf(stderr, "before open binder \n");
    ...
    if (bs->fd < 0) {
        ...
        fprintf(stderr, "open binder ok %d \n", bs->fd);
    }

    if ((ioctl(bs->fd, BINDER_VERSION, &vers) < 0) ||
        ...
        fprintf(stderr, "BINDER_VERSION ok, %d \n", vers.protocol_version);
    }
...
void binder_loop(struct binder_state *bs, binder_handler func)
{
    ...
    for (;;) {
        ...
        if (res < 0) {
            ...
            fprintf(stderr, "binder_loop: ioctl ok\n");
        }
