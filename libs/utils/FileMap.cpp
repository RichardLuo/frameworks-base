...
FileMap::~FileMap(void)
{
    ...
    if (mBasePtr && UnmapViewOfFile(mBasePtr) == 0) {
        LOGD("UnmapViewOfFile(%p) failed, error = %" PRId32 "\n", mBasePtr,
              GetLastError() );
    }
        ...
        bool readOnly)
{
    ...
    if (mFileMapping == NULL) {
        LOGE("CreateFileMapping(%p, %" PRIx32 ") failed with error %" PRId32 "\n",
    ...
    mBasePtr = MapViewOfFile( mFileMapping,
                              ...
                              adjLength );
    if (mBasePtr == NULL) {
        LOGE("MapViewOfFile(%" PRId64 ", %zu) failed with error %" PRId32 "\n",
