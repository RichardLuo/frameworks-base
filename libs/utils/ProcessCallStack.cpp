...
static void dumpProcessHeader(Printer& printer, pid_t pid, const char* timeStr) {
    if (timeStr == NULL) {
        LOGW("%s: timeStr was NULL", __FUNCTION__);
        return;
    }
...
static String8 getThreadName(pid_t tid) {
    ...
    if ((fp = fopen(path, "r"))) {
        ...
        LOGE("%s: Failed to open %s", __FUNCTION__, path);
    }
...
void ProcessCallStack::update() {
    ...
    if (dp == NULL) {
        LOGE("%s: Failed to update the process's call stacks (errno = %d, '%s')",
    ...
    while ((code = readdir_r(dp, &entry, &ep)) == 0 && ep != NULL) {
        ...
        if (tid < 0) {
            // Ignore '.' and '..'
            LOGV("%s: Failed to read tid from %s/%s",
        ...
        if (idx < 0) { // returns negative error value on error
            LOGE("%s: Failed to add new ThreadInfo (errno = %zd, '%s')",
        ...
        LOGV("%s: Got call stack for tid %d (size %zu)",
              __FUNCTION__, tid, threadInfo.callStack.size());
    }
    if (code != 0) { // returns positive error value on error
        LOGE("%s: Failed to readdir from %s (errno = %d, '%s')",
...
void ProcessCallStack::dump(int fd, int indent, const char* prefix) const {
    ...
    if (indent < 0) {
        LOGW("%s: Bad indent (%d)", __FUNCTION__, indent);
        return;
    }
